...
> This is a community-modified and namespaced build for the author's MiniMax H3
> workflows. It is not an official release of either upstream project.

This package combines adapted code from the following original projects:

- [nkxx188/ComfyUI-MiniMaxH3-Easy](https://github.com/nkxx188/ComfyUI-MiniMaxH3-Easy), by `nkxx188`;
- [yichengup/ComfyUI-YCNodes-MiniMax-H3](https://github.com/yichengup/ComfyUI-YCNodes-MiniMax-H3), by `yc` / `yichengup`.

Thank you to both original authors for publishing their work. The modified node
registration keys use the `gosick_233_` namespace so this package can coexist
with the original packages without duplicate node registrations. For the
...
This modified package is distributed under the MIT terms retained from both
upstream projects. See [LICENSE](LICENSE) for the `nkxx188` copyright notice
and [LICENSE-YCNODES](LICENSE-YCNODES) for the `yc` copyright notice.
