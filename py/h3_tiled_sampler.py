...
class H3TiledSampler:
    ...
    CATEGORY = "gosick_233/MiniMax H3/sampling"
    DESCRIPTION = (
    ...
    def sample_tiled(self, noise, guider, sampler, sigmas, latent_image,
        ...
        if audio_tensor is None:
            raise ValueError(
                "H3 分块采样需要音视频联合 latent，但输入中没有找到 audio latent。"
                "请确认 LTXVConcatAVLatent 已同时连接 video_latent 和 audio_latent。"
            )

        # H3 的扩散模型始终接收 NestedTensor([video, audio])。先针对完整的
        # 音视频 latent 生成一次噪声，再只切视频噪声；这样重叠区域使用同一份
        # 全局噪声，且每个 tile 都保留模型必需的 x[1] audio 输入。
        full_av_latent = NestedTensor([video_tensor, audio_tensor])
        full_av_noise = noise.generate_noise({"samples": full_av_latent})
        full_noise_video, full_noise_audio = full_av_noise.unbind()

        video_tensor = video_tensor.to(device=device)
        audio_for_sample = audio_tensor.to(device=device)
        full_noise_video = full_noise_video.to(device=device)
        full_noise_audio = full_noise_audio.to(device=device)

        if debug:
            print(
                f"  \u00b7 noise video={tuple(full_noise_video.shape)} "
                f"audio={tuple(full_noise_audio.shape)} on {device}"
            )
        ...
        for tile_idx, ax_start in enumerate(starts):
            ...
            # x[1] audio。音频不做空间切片，每个视频 tile 携带同一份完整音频；
            ...
            tile_samples, _tile_audio, _tile_fmt = _h3_extract(tile_result, debug)
            ...
            try:
                ...
                if hasattr(model, "process_latent_out") and "x0" in x0_output and x0_output["x0"] is not None:
                    ...
                    try:
                        x0_video, _x0_audio, _x0_fmt = _h3_extract(x0_proc, debug)
                    except TypeError:
                        x0_video = x0_proc if isinstance(x0_proc, torch.Tensor) else None
                    if isinstance(x0_video, torch.Tensor) and x0_video.shape == tile_samples.shape:
                        denoised_present = True
                        x0_video = x0_video.to(device=device)
                        if tile_axis == "H":
                            denoised_output[:, :, :, ax_start:ax_end, :] += \
                                x0_video.float() * window
                        else:
                            denoised_output[:, :, :, :, ax_start:ax_end] += \
                                x0_video.float() * window
            except Exception as e:
        ...
        del full_av_latent, full_av_noise
        del full_noise_video, full_noise_audio, audio_for_sample
...
# 注册节点
NODE_CLASS_MAPPINGS = {
    "gosick_233_H3TiledSampler": H3TiledSampler,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "gosick_233_H3TiledSampler": "gosick_233 · H3 Tiled Sampler",
}
